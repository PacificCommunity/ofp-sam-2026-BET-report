...
scenario_levels <- sort(sub("^skj_", "", model_names))
scenario_colors <- setNames(
  viridis(length(scenario_levels), option = "D", direction = 1),
  scenario_levels
)
dir <-map(results, "parent_dir")
info <-map(results, "info")
ave_bio <-map(results, "avg_bio")
...
## Total catch
```{r Catch, echo=FALSE, message=FALSE, fig.dim=c(9,5)}

CatchFits <- purrr::imap_dfr(RepOut_list, function(repout, scenario_name) {
  obs <- as.data.frame(catch_obs(repout)) %>%
    mutate(Type = "Observed")
  fit <- as.data.frame(catch_pred(repout)) %>%
    mutate(Type = "Fitted")
  ...
  bind_rows(obs, fit) %>%
    mutate(
      ...
      Catch = as.numeric(data)
    ) %>%
    filter(!is.na(Catch), Year >= minYear, Year <= maxYear) %>%
...
ObsCatch <- CatchFits %>%
  ...
  group_by(Year) %>%
  summarise(Catch = mean(Catch, na.rm = TRUE), .groups = "drop")
...
ggplot() +
  ...
  geom_line(
    data = FitCatch,
    aes(x = Year, y = Catch / 1000, colour = Scenario),
    linewidth = linewidth,
    alpha = alpha
  ) +
  geom_point(
  ...
  labs(
    x = "Year",
    y = bquote("Total catch (" * 10^3 * " model units)"),
    colour = "Scenario"
  ) +
  theme(
...
Likes_long <- Likes_df %>%
  ...
  mutate(
    scaler = as.numeric(scaler) * 0.01 * sapply(scenario, function(x) ave_bio[[x]])
  )
